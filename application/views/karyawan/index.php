            ...
            <div class="contentku container-fluid animate__animated animate__bounce animate__fadeInBottomRight">
                <div class="row">
                    <div class="judul-dalam col-12">
                        <h4 class="card-title" style="margin-top: 0.5rem;">Data Peserta Seleksi Manager</h4>
                        <div class="card-subtitle">
                            <?= anchor('karyawan/tambah', 'Tambah Data', 'class="tambah btn"') ?>
                        </div>
                        <div class="table-responsive col-12">
                            <table class="container-fluid col-12" id="dataTables1">
                                <thead>
                                    <tr>
                                        <th class="tb">No</th>
                                        <th class="tb">Periode</th>
                                        <th class="tb">NIK</th>
                                        <th class="tb">Nama</th>
                                        <th class="tb">Jenis Kelamin</th>
                                        <th class="tb">Bidang Pekerjaan</th>
                                        <th class="tb col-3" width="20%">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($karyawan as $row) : ?>
                                        <tr>
                                            ...
                                            <td class="tb-dalam"><?= $row->nik ?></td>
                                            <td class="tb-dalam"><?= $row->nama_karyawan ?></td>
                                            <td class="tb-dalam"><?= $row->jenis_kelamin ?></td>
                                            <td class="tb-dalam"><?= $row->bidang_pekerjaan ?></td>
                                            <td class="tb-dalam">
                                                <a href="<?= site_url('karyawan/detail/' . $row->id_karyawan) ?>" class="detail btn" title="Detail">Detail</a>
                                                <a href="<?= site_url('karyawan/ubah/' . $row->id_karyawan) ?>" class="ubah btn" title="Ubah">Ubah</a>
                                                <a href="#" data-href="<?php echo site_url('karyawan/hapus/' . $row->id_karyawan); ?>" data-toggle="modal" 
                                                data-target="#confirm-delete" class="hapus btn" title="Hapus">Hapus</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
